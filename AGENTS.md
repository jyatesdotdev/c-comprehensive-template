...
| `src/<module>/` | Implementations (mirrors `include/`). `include/memory` + `src/memory` compile into the `core` target — there is no `libmemory`. |
...
Modules: `core` (errors, logging, time, plus arena/pool/leak_detect from
`memory/`), `containers` (vec/hashmap/strbuf/ringbuf), `cli`, `systems`, `hpc`,
`math` (owns `Vec3`), `ml` (layers/losses/optimizers, depends on `math`),
`networking`, `simulation`, `rendering_sw` (+ optional `rendering`). Every
module depends on `core`. Do not `target_link_libraries(... memory)`.
