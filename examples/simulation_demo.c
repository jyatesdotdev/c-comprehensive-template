...
int main(void) {
    ...
    for (int i = 0; i <= 60; i++) {
        if (i % 20 == 0) {
            printf("  step %3d: p0=(%.2f,%.2f) p1=(%.2f,%.2f)\n", i, (double)parts[0].pos.x,
                   (double)parts[0].pos.y, (double)parts[1].pos.x, (double)parts[1].pos.y);
        }
        physics_step_verlet(parts, 2, pdt, gravity);
        physics_collide_spheres(&parts[0], &parts[1], 0.5f);
        for (int j = 0; j < 2; j++) physics_confine_box(&parts[j], bounds, 0.8f);
    }
    printf("  step  60: p0=(%.2f,%.2f) p1=(%.2f,%.2f)\n", (double)parts[0].pos.x,
           (double)parts[0].pos.y, (double)parts[1].pos.x, (double)parts[1].pos.y);
